"""Coalition query: seed + deterministic convergence + mocked-LLM run."""
...
def test_run_coalition_query_end_to_end(seeded_data: Path) -> None:
    client = _client_with([_canned_llm_output()])
    analysis = run_coalition_query(client, seeded_data, camp_ids=_SEED_CAMP_IDS)
...
def test_run_coalition_query_raises_when_camp_missing(
    ...
    with pytest.raises(RuntimeError, match="missing expected camps"):
        run_coalition_query(client, data)


def test_coalition_analysis_round_trips_json(seeded_data: Path, tmp_path: Path) -> None:
    client = _client_with([_canned_llm_output()])
    analysis = run_coalition_query(client, seeded_data, camp_ids=_SEED_CAMP_IDS)
    json_path, _ = analysis_paths(tmp_path / "out", analysis)
    write_analysis_json(analysis, json_path)
    reloaded = CoalitionAnalysis.model_validate_json(json_path.read_text())
...
def test_llm_output_schema_roundtrips() -> None:
    ...
    parsed = CoalitionLLMOutput.model_validate(payload)
...
def test_query_cli_requires_api_key(
    ...
    runner = CliRunner()
    monkeypatch.delenv("ANTHROPIC_API_KEY", raising=False)
    result = runner.invoke(app, ["query", "coalition"])
...
def test_bridge_id_is_content_addressed() -> None:
    ...
    Content-addressed IDs let the coalition query dedupe on re-run without the
...
def test_persist_coalition_nodes_writes_once_and_dedupes(
    seeded_data: Path,
) -> None:
    """First persist writes the Bridge/BlindSpot YAMLs; second persist is a no-op.

    Guards D1: re-running the coalition query should not flood `data/bridges/` or
    ...
    first = persist_coalition_nodes(analysis, seeded_data)
    ...
    second = persist_coalition_nodes(analysis, seeded_data)
...
def test_persisted_bridge_content_hash_roundtrips(seeded_data: Path) -> None:
    ...
    client = _client_with([_canned_llm_output()])
    analysis = run_coalition_query(client, seeded_data, camp_ids=_SEED_CAMP_IDS)
    persist_coalition_nodes(analysis, seeded_data)
