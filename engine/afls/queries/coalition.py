"""Coalition query: camps -> Bridges/Convergences/BlindSpots.
...
Camp selection is caller-controlled: pass `camp_ids=` to narrow the analysis.
The default (`DEFAULT_CAMP_IDS`) is the canonical 5-camp test case anchor
(palantir, anthropic, operator, displaced_workers, xrisk).
"""
...
class CoalitionLLMOutput(_StrictModel):
    """Complete schema the LLM must return for the coalition query."""
...
class CoalitionAnalysis(BaseModel):
    """Canonical output written to `public-output/analyses/coalition_<timestamp>.json`."""

    model_config = ConfigDict(extra="forbid")

    kind: ClassVar[str] = "coalition_analysis"
...
_USER_PROMPT = """Analyze the coalition question for this graph.
...
def persist_coalition_nodes(analysis: CoalitionAnalysis, data_dir: Path) -> dict[str, int]:
